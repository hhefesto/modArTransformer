...
-- Same attention block, but returning only sequence position 0.  The transformer
-- readout uses only this position, so this avoids constructing an unused branch.
attn0D : D (AttnParams dModel dK × (ℝVec dModel × ℝVec dModel))
             (ℝVec dModel)
attn0D {dModel} {dK} = y0
  where
    Dom : Set
